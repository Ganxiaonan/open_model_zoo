...
class Evaluator(object):
    ...
    def get_balance_mark(self):
        if self.state == 'Initial':
            ...
            object_left_score, _ = self.evaluate_object_left_from_view(view='top')
            print('condition 2:',object_left_score is not None and object_left_score >= 0)
            if object_left_score is not None and object_left_score >= 0:
                self.balance_persist_duration += 1
                print(self.frame_counter,self.balance_persist_duration)
        ...
        print('condition to evaluate balance:',self.state == "Measuring" and self.is_object_put == True and self.scoring['end_score_tidy'] == 0,self.state == "Measuring",self.is_object_put == True,self.scoring['end_score_tidy'] == 0)
    ...
    def evaluate_scale_balance(self):
        ...
        if len(roundscrew2_coor) == 2 and len(pointerhead_coor) == 1:
            ...
            print(f'scale {scale_coor}, {scale_coor[0][2] - scale_coor[0][0]}')
            print(f'use scale {pointerhead_center_coor[0] < upper_limit and pointerhead_center_coor[0] > lower_limit}:pointerhead_center_coor[0] {pointerhead_center_coor[0]} < upper_limit {upper_limit} and pointerhead_center_coor[0] {pointerhead_center_coor[0]}> lower_limit {lower_limit}')
